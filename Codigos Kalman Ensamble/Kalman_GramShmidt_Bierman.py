...
def readSignal(path, samplingRate):
    ...
    # return shape: [n_sessions, n_sensors, n_samples]
...
def run(nameSignal, Fs, wC):
    """
    Ensemble Kalman filter using Gram–Schmidt time update + Bierman measurement update.
    ...
    for session in sig:
        ...
        for j in range(Fs):
            ...
            x_pred = F @ x
            S = getNextSquareRoot(P, np.eye(m), F, kind, method='gram_schmidt')
