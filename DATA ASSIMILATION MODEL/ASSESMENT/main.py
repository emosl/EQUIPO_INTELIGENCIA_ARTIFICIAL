...
async def run_kalman_endpoint(
    ...
    try:
        # a) store ResultsY (unchanged)
        if np.isfinite(y_all_scalar) and abs(y_all_scalar) <= MAX_FLOAT32:
            _ = store_y_result(
                db=db,
                session_id=session_id,
                algorithm_id=algorithm.id,
                y_value=y_all_scalar,
                time_val=y_all_scalar,
            )

        # b) store ResultsAmp (unchanged)
        if np.isfinite(amp_all_summary) and abs(amp_all_summary) <= MAX_FLOAT32:
            _ = store_amp_result(
                db=db,
                session_id=session_id,
                algorithm_id=algorithm.id,
                amplitude=amp_all_summary,
                time_val=amp_all_summary,
            )

        # c) store ResultsWelch (use welch_value instead of y_value)
        if np.isfinite(welch_summary) and abs(welch_summary) <= MAX_FLOAT32:
            _ = store_welch_result(
                db=db,
                session_id=session_id,
                algorithm_id=algorithm.id,
                welch_val=welch_summary,   # ← changed param name
    ...
    # 10) return RunResponse (unchanged)
    return RunResponse(
